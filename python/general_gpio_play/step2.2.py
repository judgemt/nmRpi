...
## Main loop to handle motor stepping based on button press
try:
    steps = 0
    ...
    while True:
        ...
        if GPIO.input(buttonPin) == GPIO.LOW:  # Button pressed (PUD_UP makes LOW = pressed)
            ...
            time.sleep(0.25)  # Debounce the button (adjustable)

        # Sleep for a short duration to reduce CPU usage
        time.sleep(0.1)

except KeyboardInterrupt:
    # Handle specific exception, like pressing Ctrl+C
    print("Interrupted by user, exiting...")

finally:
    # Ensure GPIO cleanup no matter what
    GPIO.cleanup()  # Only one cleanup call for both stepper and pin_reader
